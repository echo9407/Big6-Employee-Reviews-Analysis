...
df = read.csv('desktop/employee_reviews.csv', stringsAsFactors = FALSE)
str(df)
library(dplyr)
...
library(corrplot)
df1 <- na.omit(df)
corrplot(cor(df1[,9:14]),method='color',type='lower',addCoef.col = "gray8",diag = T)
...
#From the picture we can see the proportion of positive words in summary is positively correlated to the overall ratings.
# Let's quantify this relationship with the whole dataset to answer the question:
...
dict = findFreqTerms(DocumentTermMatrix(Corpus(VectorSource(df$summary))),lowfreq = 0)
...
df_data = cbind(overall_rating = df$overall.ratings,xdtm)
...
rmse_tree = sqrt(mean((pred_tree - test$overall_rating)^2)); rmse_tree

reg = lm(overall_rating~.,train)
pred_reg = predict(reg, newdata=test)
rmse_reg = sqrt(mean((pred_reg-test$overall_rating)^2)); rmse_reg
